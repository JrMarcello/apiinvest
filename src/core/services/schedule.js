        ...
        try {
            ...
            const fundraisings = await Fundraising.findAll({
                where: {
                    status: {
                        [Sequelize.Op.or]: [
                            statuses.fundraising.OPENED,
                            statuses.fundraising.CONFIRMED
                        ]
                    }
                }
            });
            ...
            if (fundraisings.length > 0) {
                ...
                if (closed.length > 0) {
                    await Fundraising.update({ status: statuses.fundraising.CLOSED }, {
                        where: {
                            id: {
                ...
                if (canceled.length > 0) {
                    await Fundraising.update({ status: statuses.fundraising.CANCELED }, {
                        where: {
                            id: {
                    ...
                    await Investment.update({ status: statuses.investment.WAITING_RETURN }, {
                        where: {
                            id_fundraising: {
                                [Sequelize.Op.or]: canceled
                            }
